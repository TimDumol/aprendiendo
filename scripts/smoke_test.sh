...
docker run -d --name "$NAME" \
  ...
  -p 127.0.0.1:8080:8080 aprendiendo-mcp:linux >/dev/null
...
python3 -c 'import json,sys; names=[x["name"] for x in json.load(sys.stdin)["result"]["tools"]]; assert len(names)==9 and "get_taxonomy" in names and "upsert_concept" in names, names; print(names)' <<<"$tools"
